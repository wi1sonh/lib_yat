...
{
	union yat_syscall_args args;
	args.get_set_task_param.pid = pid;
	args.get_set_task_param.param = param;
	return yat_syscall(LRT_set_rt_task_param, (unsigned long) &args);
}
...
int get_rt_task_param(pid_t pid, struct rt_task *param)
{
	...
	args.get_set_task_param.pid = pid;
	args.get_set_task_param.param = param;
	return yat_syscall(LRT_get_rt_task_param, (unsigned long) &args);
}
...
int reservation_create(int rtype, void *config)
{
	...
	return yat_syscall(LRT_reservation_create, (unsigned long) &args);
}

int sleep_next_period(void)
{
	return yat_syscall(LRT_complete_job, 0);
}
...
int od_openx(int fd, obj_type_t type, int obj_id, void *config)
{
	...
	return yat_syscall(LRT_od_open, (unsigned long) &args);
}

int od_close(int od)
{
	return yat_syscall(LRT_od_close, od);
}

int yat_lock(int od)
{
	return yat_syscall(LRT_yat_lock, od);
}

int yat_unlock(int od)
{
	return yat_syscall(LRT_yat_unlock, od);
}
...
{
	return yat_syscall(LRT_wait_for_job_release, job_no);
}

int wait_for_ts_release(void)
{
	return yat_syscall(LRT_wait_for_ts_release, 0);
}

int release_ts(lt_t *when)
{
	return yat_syscall(LRT_release_ts, (unsigned long) when);
}

int null_call(cycles_t *timestamp)
{
	return yat_syscall(LRT_null_call, (unsigned long) timestamp);
}
	...
	lt_t *remaining)
{
	...
	return yat_syscall(LRT_get_current_budget, (unsigned long) &args);
}
