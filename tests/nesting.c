	 ...
	 "FMLP no nesting allowed")
{
	...
	SYSCALL( yat_lock(od2) );
	SYSCALL( yat_unlock(od2) );

	SYSCALL( yat_lock(od) );
	SYSCALL_FAILS(EBUSY, yat_lock(od2));
	SYSCALL( yat_unlock(od) );

	SYSCALL( yat_lock(od2) );
	SYSCALL_FAILS(EBUSY, yat_lock(od));
	SYSCALL( yat_unlock(od2) );
	 ...
	 "PCP nesting allowed")
{
	...
	SYSCALL( od  = yat_open_lock(PCP_SEM, 0, namespace, NULL) );
	SYSCALL( od2 = yat_open_lock(PCP_SEM, 1, namespace, NULL) );
	 ...
	 "PCP and MPCP nesting not allowed")
{
	...
	SYSCALL( od  = yat_open_lock(PCP_SEM, 0, namespace, NULL) );
	SYSCALL( od2 = yat_open_lock(MPCP_SEM, 1, namespace, NULL) );
	 ...
	 "PCP and FMLP nesting not allowed")
{
	...
	SYSCALL( od  = yat_open_lock(PCP_SEM, 0, namespace, NULL) );
	SYSCALL( od2 = yat_open_lock(FMLP_SEM, 1, namespace, NULL) );
	 ...
	 "PCP and DPCP nesting not allowed")
{
	...
	SYSCALL( od  = yat_open_lock(PCP_SEM, 0, namespace, NULL) );
	SYSCALL( od2 = yat_open_lock(DPCP_SEM, 1, namespace, &cpu) );
	 ...
	 "SRP and MPCP nesting not allowed")
{
	...
	SYSCALL( od  = yat_open_lock(SRP_SEM, 0, namespace, NULL) );
	SYSCALL( od2 = yat_open_lock(MPCP_SEM, 1, namespace, NULL) );
	 ...
	 "MPCP and FMLP nesting not allowed")
{
	...
	SYSCALL( od  = yat_open_lock(MPCP_SEM, 0, namespace, NULL) );
	SYSCALL( od2 = yat_open_lock(FMLP_SEM, 1, namespace, NULL) );
	 ...
	 "DPCP and FMLP nesting not allowed")
{
	...
	SYSCALL( od  = yat_open_lock(DPCP_SEM, 0, namespace, &cpu) );
	SYSCALL( od2 = yat_open_lock(FMLP_SEM, 1, namespace, NULL) );
	 ...
	 "MPCP no nesting allowed")
{
	...
	SYSCALL( od  = yat_open_lock(MPCP_SEM, 0, ".mpcp_locks", NULL) );
	SYSCALL( od2 = yat_open_lock(MPCP_SEM, 1, ".mpcp_locks", NULL) );
	 ...
	 "MPCP-VS no nesting allowed")
{
	...
	SYSCALL( od  = yat_open_lock(MPCP_VS_SEM, 0, ".mpcp_locks", NULL) );
	SYSCALL( od2 = yat_open_lock(MPCP_VS_SEM, 1, ".mpcp_locks", NULL) );
	 ...
	 "DPCP no nesting allowed")
{
	...
	SYSCALL( od  = yat_open_lock(DPCP_SEM, 0, ".dpcp_locks", &cpu) );
	SYSCALL( od2 = yat_open_lock(DPCP_SEM, 1, ".dpcp_locks", &cpu) );
	 ...
	 "DFLP no nesting allowed")
{
	...
	SYSCALL( od  = yat_open_lock(DFLP_SEM, 0, ".dflp_locks", &cpu) );
	SYSCALL( od2 = yat_open_lock(DFLP_SEM, 1, ".dflp_locks", &cpu) );
	 ...
	 "DFLP and FMLP nesting not allowed")
{
	...
	SYSCALL( od  = yat_open_lock(DFLP_SEM, 0, namespace, &cpu) );
	SYSCALL( od2 = yat_open_lock(FMLP_SEM, 1, namespace, NULL) );
	 ...
	 "PCP and DFLP nesting not allowed")
{
	...
	SYSCALL( od  = yat_open_lock(PCP_SEM, 0, namespace, NULL) );
	SYSCALL( od2 = yat_open_lock(DFLP_SEM, 1, namespace, &cpu) );
