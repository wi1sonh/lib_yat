	 ...
	 "reject invalid priorities")
{
	...
	params.priority	  = 0;
	SYSCALL( set_rt_task_param(gettid(), &params) );
	SYSCALL_FAILS( EINVAL, task_mode(YAT_RT_TASK) );

	/* too low */
	params.priority   = YAT_MAX_PRIORITY;
	SYSCALL( set_rt_task_param(gettid(), &params) );
	SYSCALL_FAILS( EINVAL, task_mode(YAT_RT_TASK) );
	 ...
	 "accept lowest and highest valid priorities")
{
	...
	params.priority   = YAT_HIGHEST_PRIORITY;
	SYSCALL( set_rt_task_param(gettid(), &params) );
	SYSCALL( task_mode(YAT_RT_TASK) );
	SYSCALL( task_mode(BACKGROUND_TASK) );
}
...
TESTCASE(ctrl_page_writable, ALL,
	 "tasks have write access to /dev/yat/ctrl mappings")
{
	volatile int *ctrl_page = (volatile int*) get_ctrl_page();

	/* init_yat() should have mapped the page already  */
...
TESTCASE(suspended_admission, YAT,
	 "admission control handles suspended tasks correctly")
{
	...
	child_rt = FORK_TASK(
		...
		ASSERT( sched_getscheduler(child_rt) == SCHED_YAT );
	...
	SYSCALL( sched_setscheduler(child_rt, SCHED_YAT, &param) );
...
TESTCASE(running_admission, YAT,
