...
static void attach_task(int attach_pid, struct reservation_config *config)
{
	...
	ret = sched_setscheduler(attach_pid, SCHED_YAT, &linux_param);
	if (ret < 0) {
		fprintf(stderr, "failed to transition task %d to YAT^RT class (%m)\n",
			attach_pid);
		exit(3);
	}
}
...
int main(int argc, char** argv)
{
	...
	config.id = 0;
	config.priority = YAT_NO_PRIORITY; /* use EDF by default */
	config.cpu = 0;
	...
	while ((opt = getopt(argc, argv, OPTSTR)) != -1) {
		...
		case 't':
			if (strcmp(optarg, "polling-periodic") == 0) {
				...
				if (config.priority == YAT_NO_PRIORITY)
					config.priority = YAT_HIGHEST_PRIORITY;
			} else {
